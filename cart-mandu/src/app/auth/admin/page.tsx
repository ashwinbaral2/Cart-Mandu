...
export function DeleteDialog(props: DeleteDialogProps) {
    ...
    const handleDelete = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const { data } = await axios.delete("https://fakestoreapi.com/products/" + props.item.id
            );
            if (data) {
    ...
    if (isDesktop) {
        return (
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                    {props.action === 'edit' ? <EditIcon /> : <Button className="mb-4 ml-4 bg-green-800">Add Products</Button>}
                </DialogTrigger>
                <DialogContent className="sm:max-w-106.25">
                    <DialogHeader>
                        {props.action === 'edit' ? <DialogTitle> Edit Products</DialogTitle> : <DialogTitle> Add Products</DialogTitle>}
                        <DialogDescription>
                            {props.action === 'edit' ? 'Edit product' : 'Add new product '}
                        </DialogDescription>
                    </DialogHeader>
...
function ProfileForm(props: ProfileFormProps = {}) {
    ...
    const handleEdit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const { data } = await axios.put("https://fakestoreapi.com/products/" + props.item.id,
                formData
            );
            if (data) {
    ...
    return (
        <form onSubmit={props.item ? handleEdit : handlePost} className={cn("grid items-start gap-6")}>
